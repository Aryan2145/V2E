...
import { getPath, updatePath } from '@/lib/api/learning'
...
export default function EditLearningPathPage() {
  ...
  useEffect(() => {
    ...
    getPath(orgId, pathId).then((path) => {
      setForm({
        title: path.title,
        description: path.description ?? '',
        mode: path.mode,
        estimated_minutes: path.estimated_minutes?.toString() ?? '',
      })
  ...
  return (
    <div className="px-8 py-8 max-w-2xl mx-auto">
      ...
      <h1 className="text-[28px] font-bold text-[#0F172A] mb-6">Edit Learning Path</h1>
