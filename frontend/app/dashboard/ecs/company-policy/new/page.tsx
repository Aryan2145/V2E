...
export default function NewPolicyPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('')
    try {
      const policy = await createPolicy(orgId, {
        title: form.title.trim(),
        description: form.description.trim() || undefined,
      })
      router.push(`/dashboard/ecs/company-policy/${policy.id}`)
    } catch (err: any) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <Link
        href="/dashboard/ecs/company-policy"
      ...
      <h1 className="text-[28px] font-bold text-[#0F172A] mb-6">Create Company Policy</h1>
