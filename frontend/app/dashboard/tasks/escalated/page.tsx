    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-[15px] text-[#475569]">
          Tasks that have been escalated and require immediate attention.
        </p>
      </div>
      ...
      <p className="text-sm text-[#475569]">
        {filtered.length} escalated task{filtered.length !== 1 ? 's' : ''}
      ...
      {filtered.length === 0 ? (
        <div className="bg-white border border-[#E2E8F0] rounded-[12px] shadow-[0_1px_3px_rgba(0,0,0,0.08)] flex flex-col items-center justify-center py-20">
          ...
          <p className="font-semibold text-[#0F172A]">{isFiltered ? 'No tasks match your filters' : 'No escalated tasks'}</p>
          <p className="text-sm text-[#475569] mt-1">{isFiltered ? 'Try adjusting your filters.' : 'Tasks flagged for escalation will appear here.'}</p>
        </div>
      ) : (
