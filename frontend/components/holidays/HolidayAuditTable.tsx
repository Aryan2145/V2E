  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-[12px] shadow-[0_1px_3px_rgba(0,0,0,0.08)] overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3 border-b border-[#E2E8F0]">
        <p className="text-sm font-semibold text-[#0F172A]">{logs.length} record{logs.length !== 1 ? 's' : ''}</p>
        <button
          type="button"
          onClick={exportCsv}
          className="flex items-center gap-1.5 h-8 px-3 rounded-[8px] border border-[#E2E8F0] text-xs font-medium text-[#475569] hover:bg-[#F8FAFC] transition-colors"
        >
          <Download size={13} />
          Export CSV
        </button>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
            <tr>
              {['Date', 'Entity', 'Title', 'Holiday', 'Action', 'Original', 'Adjusted'].map((h) => (
                <th key={h} className="px-4 py-2.5 text-left text-xs font-semibold text-[#475569] whitespace-nowrap">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-[#E2E8F0]">
            {logs.map((l) => (
              <tr key={l.id} className="hover:bg-[#F8FAFC] transition-colors">
                ...
                <td className="px-4 py-3 text-sm text-[#0F172A] max-w-[200px] truncate">{l.entity_title ?? '—'}</td>
                <td className="px-4 py-3 text-sm text-[#475569]">{l.holiday_name ?? '—'}</td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 text-xs text-[#475569] whitespace-nowrap">
                  {new Date(l.original_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                </td>
                <td className="px-4 py-3 text-xs text-[#475569] whitespace-nowrap">
                  {l.adjusted_date
